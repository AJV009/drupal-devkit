...
- **Explore when things fail.** "Access denied" is information, not a wall. Browse admin menus, try adjacent paths, research the platform. Acquia Source is not standard Drupal — discover how it differs.
...
When something fails or is unexpected, **investigate before logging as blocked**. This is a migration, not a checklist — the source platform may work differently than expected.

- **"Access denied" means try another path.** Acquia Source admin paths differ from standard Drupal. If `/admin/config/system/site-information` returns 403, browse `/admin/config/system/` to discover what's available. Try `/admin/config/system/site-settings`.
...
- **Use all available tools.** Browser automation, Canvas Code Editor, Source MCP, Storybook, Playwright, `npm run content` — each gives a different view. When one fails, try another.
- **Research the platform.** Acquia Source is a customized Drupal distribution. It has its own module configurations, permissions model, and admin UX. Don't assume standard Drupal behavior.
- **Try at least 3 alternative approaches** before logging something as blocked. Document what you tried in `decisions.md`.
- **Check the source for clues.** If a feature seems missing, inspect what modules are enabled, what admin paths exist, or what the Source MCP exposes.

## Canvas Editor

Acquia Source provides web-based editors for inspecting deployed components and composing pages.
...
2. **Components exist**: Check `src/components/` has components with `component.yml` and `index.jsx` files.
3. **Source connection**: Run `npm run canvas:validate` to confirm Canvas CLI can connect. If it fails, check `.env` configuration.
4. **Content API**: Run `npm run content -- list --types` to confirm JSON:API access works. Verify write access by checking that `menu_items--main` and `page` types are listed.
5. **Content API write access**: Run `npm run content -- list page` to verify read works. If writes later fail with HTTP 405, JSON:API is in read-only mode — see content-management skill for fix.

If pre-flight fails, log the failure to `docs/migration/decisions.md` and attempt to fix it. Do not stop.
...
- **Phase 3**: `component-builder` fetches docs on component structure, props, slots, and known issues via `acquia-source-docs-explorer`
- **Phase 3.5**: `component-fixer` fetches docs on component styling and known issues via `acquia-source-docs-explorer`; `storybook-qa` does not fetch docs (uses design-tokens.md and css-audit.md as ground truth)
...
- **503 recovery**: `upload-verifier` fetches docs on known issues when needed
...
- Path to `src/components/` (all existing components)
...
### Phase 3: Build Missing Components (with Storybook Structural Verification)
...
5. Verify the component renders structurally (not blank, correct element count, correct general layout).
6. Do NOT iterate on color accuracy, spacing precision, or font matching — Phase 3.5 handles visual fidelity systematically.
7. Run `canvas:validate` and `canvas:ssr-test`.

**Storybook Atomic Structure:**

All stories must be placed in the correct atomic level:
- `src/stories/atoms/` — button, heading, text, image, spacer, logo, video
- `src/stories/molecules/` — card, blockquote, logo_card, search_form, search_button, breadcrumb
- `src/stories/organisms/` — hero, card_container, two_column_text, grid_container, stats_banner, contact_section, header, footer, main_navigation, section
- `src/stories/templates/` — PageLayout
- `src/stories/pages/` — Full page compositions (Home, Services, About, Contact)

The component-builder agent creates stories at the correct level with VERBATIM content from `docs/migration/content/`. After individual component stories pass structural verification, compose full-page stories in `src/stories/pages/` and verify sections are present in correct order.
...
The agent will:
1. Run `npm run canvas:preflight` to validate all components locally — fix any failures
2. Upload all components: `npm run canvas:upload`
...
2. Upload each to Acquia Source media library via `npm run content -- upload`
...
- The CMS URL (from `.env` `CANVAS_SITE_URL`)
- The public site URL (from `.env` `CANVAS_PUBLIC_URL`)
...
The agent handles:
- Setting site name via `/admin/config/system/site-settings` (NOT standard Drupal paths — Acquia Source uses a custom settings page)
- Uploading logo and favicon via file upload fields on the same page
...
- Verifying each page loads at its expected URL on the public site
...
- If CDN cache issue detected (menus show stale data on live site), apply the `cache: 'no-store'` fix to the navigation component (see Troubleshooting section: "Menu Items Not Showing in Header/Footer")
...
2. The content-composer reads all inputs and composes JSON with EXACT text from the content file. It deploys the page via `npm run content -- create` or `-- update`.
...
3. **After `content-composer` finishes page N**, spawn `phase-verifier` for page N in background (`run_in_background: true`, mode `page-check`) with:
   - Source page URL
   - Target page URL (from `CANVAS_PUBLIC_URL` in `.env`, with `?cb=<timestamp>` cache buster)
...
1. **Check the component in Code Editor:** Navigate to `<CMS_URL>/canvas/code-editor/component/<name>` — this shows props, source code, and a live preview regardless of component status.
2. **Verify the API still works:** `npm run content -- list page`. If this returns data, the backend is fine.
3. **Run local validation:** `npm run canvas:validate`. If all components pass locally, the issue is likely in the page data or a runtime error in the component JavaScript.
...
1. **If a specific component is suspect**, re-upload just that component: `npm run canvas:upload -- -c <name>`. Verify in Code Editor.
2. **If re-uploading doesn't fix it**, the issue may be in the page data. Fetch the page (`npm run content -- get page <uuid>`), inspect the `components` array for malformed inputs, fix them, and update.
3. **If nothing works**, check `blocked.md` and consult Acquia Source documentation via `/acquia-source-docs-explorer known issues site unavailable`.

### Prevention

- **Always validate before uploading:** `npm run canvas:validate` catches structural issues. But it does NOT catch runtime JavaScript errors (like null pointer exceptions).
- **Add null guards** for all optional props that get destructured — especially image, link, and formatted text props.
- **Verify in Storybook first.** Build and preview in Storybook with realistic data AND with missing/empty data before uploading.
- **Check the Code Editor after uploading.** Navigate to `<CMS_URL>/canvas/code-editor/component/<name>` to verify props and preview render correctly.
...
1. **Webforms / contact forms** — Acquia Source may not support webforms. Leave a placeholder component (e.g., a `contact_section` with static info) and log in `blocked.md` as out-of-scope.
...
Invoke with: `/acquia-source-docs-explorer <query>`
