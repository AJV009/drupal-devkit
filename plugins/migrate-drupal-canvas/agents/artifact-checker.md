...
| Check | Command | Pass Criteria |
|-------|---------|---------------|
| Component files exist | `ls canvas/src/components/*/index.jsx canvas/src/components/*/component.yml` | Both files exist for each component |
| Stories exist | `ls canvas/src/stories/**/*.stories.tsx` | Story file exists for each component |
| Canvas validates | `cd canvas && npm run canvas:validate` | Exit code 0 |
| Page stories exist | `ls canvas/src/stories/pages/*.stories.tsx` | At least one page story per migrated page |
...
| Preflight passes | `cd canvas && npm run canvas:preflight` | Exit code 0 |
...
| All components accounted for | Compare progress.md table against `ls canvas/src/components/` | Every component has a status (enabled, blocked, or skipped with reason) |
