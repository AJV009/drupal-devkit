...
1. **Read references:**
   - Source section screenshot from `docs/migration/screenshots/pages/<page>/desktop/section-NN-<name>.png` (and mobile)
   - CSS audit for that section from `docs/migration/css-audit.md`
   - Design tokens from `docs/migration/design-tokens.md`
   - Verbatim content from `docs/migration/content/<page>.md`
   - Any existing similar component code for patterns
   - For complex components, fetch Canvas docs: `/canvas-docs-explorer components props slots known-issues-and-limitations`
...
2. **Build the component:**
   ...
   - Follow existing component patterns — use `cn()`, `cva()`, `FormattedText`, `SWR`, `JsonApiClient` as appropriate
...
3. **Create Storybook story** at the correct atomic level:
   ...
   - Use VERBATIM text from content files for story args
...
5. **Full-page Storybook composition:**
   ...
   a. Compose the page story in `canvas/src/stories/pages/<page>.stories.tsx`
...
6. **Validate:**
   - Run `cd canvas && npm run code:fix`
   - Run `cd canvas && npm run canvas:validate -- -- -c <component_name>`
   - Run `cd canvas && npm run canvas:ssr-test`
...
- Use design tokens from `docs/migration/design-tokens.md` and the `@theme` block in `canvas/src/components/global.css`
- Match breakpoint behavior from `docs/migration/css-audit.md`
- Use exact color values — not approximations
- If `global.css` `@theme` is missing tokens, update it
...
- [ ] Enum values in `component.yml` match the CVA variant keys exactly
- [ ] `cd canvas && npm run canvas:validate` passes
- [ ] `cd canvas && npm run canvas:ssr-test` passes
