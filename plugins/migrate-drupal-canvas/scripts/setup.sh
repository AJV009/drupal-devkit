#!/usr/bin/env bash
# setup.sh — Bootstrap a Drupal CMS + Canvas project for site migration
...
CANVAS_DIR="$PROJECT_ROOT/canvas"
if [ -d "$CANVAS_DIR" ]; then
  cd "$CANVAS_DIR"
  MISSING_DEPS=""
  [ -d "node_modules/dotenv" ] || MISSING_DEPS="$MISSING_DEPS dotenv"
  [ -d "node_modules/drupal-jsonapi-params" ] || MISSING_DEPS="$MISSING_DEPS drupal-jsonapi-params"
  [ -d "node_modules/js-yaml" ] || MISSING_DEPS="$MISSING_DEPS js-yaml"

  if [ -n "$MISSING_DEPS" ]; then
    echo "   Installing:$MISSING_DEPS"
    npm install --save $MISSING_DEPS 2>/dev/null
  ...
  echo "   ⚠ canvas/ directory not found — run 'npx @drupal-canvas/create' first"
fi

# 5. Validate environment
echo "5. Validating environment..."
command -v ddev >/dev/null 2>&1 && echo "   ✓ ddev" || echo "   ✗ ddev (required for Drupal CMS)"
...
if [ -d "$CANVAS_DIR/node_modules/@drupal-canvas/cli" ]; then
  echo "   ✓ canvas CLI"
else
  echo "   ⚠ canvas CLI not found in canvas/node_modules/"
fi

if [ -d "$CANVAS_DIR/.storybook" ]; then
  echo "   ✓ storybook config"
else
  echo "   ⚠ .storybook/ not found in canvas/"
fi
...
echo "  2. Verify .mcp.json has correct URLs for your DDEV site"
echo "  3. Start migration: /migrate-drupal-canvas:migrate-site <source-url>"
