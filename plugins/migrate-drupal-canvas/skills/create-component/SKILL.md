...
1. Browse `canvas/src/components/` to find a similar component that can serve as a
   starting point (e.g., use `blockquote` as a base for an "alert" component, or
   `button` for any interactive element)
2. Copy the component folder to `canvas/src/components/<new_name>/`
3. Copy the corresponding story from `canvas/src/stories/`
...
```bash
cp -r canvas/src/components/blockquote canvas/src/components/alert
cp canvas/src/stories/components/blockquote.stories.tsx canvas/src/stories/components/alert.stories.tsx
```

Then modify the copied files to implement the Alert component.

Components use the `@/components` import alias, which points to
`canvas/src/components`. When you copy and modify exisiting components, the imports
...
3. For each imported component, check if it already exists in `canvas/src/components/`
...
- Check `canvas/src/components/heading/` — exists? Skip. Missing? Copy.
- Check `canvas/src/components/button/` — exists? Skip. Missing? Copy.
...
**Always check `canvas/src/components/` before creating new UI elements.** When
...
4. A Storybook story file — in `canvas/src/stories/components/`
...
7. **Validate** — run `cd canvas && npm run code:fix` (Prettier + ESLint + Canvas requirements), then `cd canvas && npm run canvas:validate`, then `cd canvas && npm run canvas:ssr-test`
8. **Upload** — run `cd canvas && npm run canvas:upload -- -- -c <name>` only after validation passes

## Required component folder structure

**CRITICAL:** Every component folder in `canvas/src/components/` MUST contain two
files:

```
canvas/src/components/<component_name>/
...
ls -la canvas/src/components/*/

# Verify each new component has both required files
ls canvas/src/components/<component_name>/index.jsx
ls canvas/src/components/<component_name>/component.yml
```
...
`canvas/src/stories/components/`. Where appropriate, internal components should also
have a story file.
