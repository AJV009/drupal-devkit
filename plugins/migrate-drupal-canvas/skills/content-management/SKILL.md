...
```bash
cd canvas && npm run content -- <command> [args...]
```
...
```bash
cd canvas && npm run content -- list <type>
cd canvas && npm run content -- list --types  # Discover available types
```
...
```bash
cd canvas && npm run content -- get <type> <uuid> [<uuid>...]
cd canvas && npm run content -- get <type> <uuid> --include <relationships>
```

Examples:

```bash
cd canvas && npm run content -- get page abc-123-def
cd canvas && npm run content -- get page abc-123 def-456 ghi-789
cd canvas && npm run content -- get media--image uuid1 uuid2 uuid3
cd canvas && npm run content -- get page abc-123-def --include image,owner
```
...
```bash
cd canvas && npm run content -- create <file-path>
```
...
```bash
cd canvas && npm run content -- update <file-path>
```
...
```bash
cd canvas && npm run content -- delete <type> <uuid> [<uuid>...]
```

Examples:

```bash
cd canvas && npm run content -- delete page abc-123-def
cd canvas && npm run content -- delete media--image uuid1 uuid2 uuid3
```
...
```bash
cd canvas && npm run content -- upload <image-path> [alt-text]
```

Example:

```bash
cd canvas && npm run content -- upload src/stories/assets/photo.jpg "Photo description"
```
...
```bash
cd canvas && npm run content -- uuid        # Generate 1 UUID
cd canvas && npm run content -- uuid 5      # Generate 5 UUIDs
```
...
```bash
cd canvas && npm run content -- upload image.jpg "Alt text"
```
...
```bash
cd canvas && npm run content -- list page
cd canvas && npm run content -- get page abc-123-def
# Edit content/page/abc-123-def.json
cd canvas && npm run content -- update content/page/abc-123-def.json
```
...
1. Upload images:
   ...
   ```bash
   cd canvas && npm run content -- upload image1.jpg "Description"
   # Note target_id: 31
   ```
...
2. Generate UUIDs:
   ...
   ```bash
   cd canvas && npm run content -- uuid 3
   ```
...
4. Create the page:
   ...
   ```bash
   cd canvas && npm run content -- create content/page/new-my-page.json
   ```
...
1. **JSON:API read-only mode**: If all write operations return HTTP 405, JSON:API is in read-only mode. Enable with: `ddev drush config:set jsonapi.settings read_only false -y`

2. **Wrong ID type**: Using file's `drupal_internal__target_id` instead of media's internal ID causes "image.src NULL value found" errors.

3. **Missing langcode permission**: When creating pages, omit the `langcode` field as the API may reject it with permission errors.

4. **Patching limitations**: PATCH requests may not work for all fields. Create a new page with a different alias if updates fail.

5. **Link props are plain strings, not objects**: Link/URL props (`format: uri-reference`) must be plain strings like `"/about"`. Do NOT use Drupal's link field format `{ "uri": "/about", "options": [] }` — that is for Drupal entity fields, not Canvas component inputs.
...
6. **Formatted text fields need the wrapper object**: Any prop with `contentMediaType: text/html` in `component.yml` must use the formatted text object, not a plain string. Omitting the wrapper may cause rendering failures or empty output.
...
7. **target_id must be a string**: Image references use `"target_id": "31"` (a string), not `"target_id": 31` (an integer). Using an integer may cause type errors.
...
```bash
cd canvas && npm run content -- list menu_items--main
cd canvas && npm run content -- list menu_items--footer
cd canvas && npm run content -- list menu_items--social-media
```
...
**Important:** Ensure page paths match the source site exactly. Use `/about` not `/about-us`. Verify aliases at `<CMS_URL>/admin/config/search/path`. Standard Drupal admin paths apply: use `/admin/config/system/site-information` for site name, `/admin/appearance/settings` for theme logo/favicon.
...
Always check the docs before assuming standard Drupal JSON:API behavior — Drupal CMS may have platform-specific differences.
...
- `CANVAS_SITE_URL` - Base URL of Drupal CMS site
